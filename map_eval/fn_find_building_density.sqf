...
private _pcb_fnc_building_classifier = compile preprocessFileLineNumbers "fn_building_classifier.sqf";
        ...
        if (((_pos select 0) > 0) || ((_pos select 1) > 0)) then {
            ...
            if ((isNil "_n_pos") || (_n_pos < 1)) then {
                ...
                if (isNil "_my_class") then {
                    _my_class = [_x] call _pcb_fnc_building_classifier; // !!!!!! FIX ME !!!!!
